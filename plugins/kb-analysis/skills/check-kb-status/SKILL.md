...
description: |
  ...
  - User invokes /kb:check-kb-status
...
# Check if KB exists
if [ ! -d "kb" ]; then
  echo "❌ KB not initialized. Run /kb:start-knowledge-base to begin."
  exit 0
fi
```
...
╠═══════════════════════════════════════════════════════════════╣
║                                                                ║
║  /kb:start-knowledge-base      - Start/resume autonomous learning             ║
║  /kb:learn-kb-topic X    - Learn specific topic X                       ║
...
Options:
1. /kb:start-knowledge-base - Resume from where you left off
2. /kb:learn-kb-topic {topic} - Restart current topic
...
|-------|----------------|
| No KB | `/kb:start-knowledge-base` to initialize |
| No sources | Add repos/docs to `kb/sources/` |
| No plan | `/kb:start-knowledge-base` will create plan |
| Has pending topics | `/kb:start-knowledge-base` to continue learning |
| All topics done | `/kb:distill-kb-knowledge` to generate output |
...
| Command | Purpose |
|---------|---------|
| `/kb:start-knowledge-base` | Initialize and start autonomous learning |
| `/kb:check-kb-status` | Show this status dashboard |
| `/kb:learn-kb-topic <topic>` | Learn specific topic manually |
| `/kb:distill-kb-knowledge` | Generate final knowledge base |
| `/kb:add-kb-source <path>` | Add source files |
